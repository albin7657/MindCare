...
router.post("/", async (req, res) => {
  try {
    const { domain_id, assessment_type_id, question_text, option_set_id, weight } = req.body;
    ...
    if (!option_set_id) {
      return res.status(400).json({ error: "Option set is required" });
    }
    ...
    const question = await Question.create({ domain_id, assessment_type_id, question_text, option_set_id, weight: weight || 1 });
    res.json(question);
  } catch (err) {
...
router.get("/domain/:domainId", async (req, res) => {
  try {
    ...
    }).populate(["assessment_type_id", "option_set_id"]);
...
router.delete("/:id", async (req, res) => {
  try {
    // Delete the question (options are now managed separately via option sets)
